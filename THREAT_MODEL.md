...
| --- | --- |
| Teaching examples | hello.py:1-6; callable.py:25-41; new_type.py:4-16 |
| Development tool installation | pyproject.toml:1-17 |
...
| Local uv environment | Development tool installation | pyproject dev group; minimum versions; uv exclude-newer | basedpyright and ruff from configured package resolution; no runtime dependencies | Developer machine and package source | uv minimum and seven-day cutoff documented; registry and resolved lock are separate | pyproject.toml:1-17 |
...
Third-party development packages → executable tools in the local environment (`pyproject.toml:9-17`).
...
Example type declaration → learner adoption in a future program; the supplied UserId example does not implement access checks (`new_type.py:4-16`).
...
No network listener, persistence store, deployment manifest, or secret consumer appears in supplied source/inventory.
...
| --- | --- | --- | --- | --- | --- | --- |
| P1 | Malicious development package code runs when a learner installs tooling. | Compromised selected package/version or accepted dependency-source change and actual installation. | Local developer account compromise, conditional on supply chain failure. | No runtime dependencies; uv cutoff reduces newly published version exposure but is not authentication. | Use reviewed resolved dependencies and verify package source before execution. | pyproject.toml:7-17 |
